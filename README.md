...
- v2.73 fixes the false-proof zoomed pan regression: one-finger horizontal
  viewer handoff now replays the original ACTION_DOWN instead of synthesizing a
  DOWN at the first MOVE point, so WebView receives real left/right displacement
  while pinch still keeps the synthetic-current handoff.
...
- v2.49 keeps the same row/canvas scrubber alive after normal toolbar-only actions
  such as Send/Enter, option keys, and touch-bottom recovery. WHY: the 2026-06-18
  22:41 proof showed the lower dotted xterm field can return after ordinary live
  work even when Active/Bottom entry proof is clean; these paths refit or align
  xterm without passing through Bottom-core, so they must re-arm the bounded
  no-reload/no-hidden-focus/no-IME/no-broad-mask scrubber too.
- v2.50 keeps Android Old Sessions on the same `/sessions?oldOnly=1` saved-row
...
- v2.55 removes success Toast overlays from Bottom and scroll visual recovery.
  WHY: the v2.54 APK proved the prompt rows existed in the capture renderer,
  but Android's "At live bottom"/Page success Toast covered the prompt area and
  recreated the "can't see the bottom text" regression. Failures still toast;
  success navigation stays visually quiet so the terminal remains readable.
- v2.56 suppresses normal Send/Start success Toasts, keeps error Toasts, and
...
- v2.59 smooths APK one-finger history scrolling without changing the protected
  slow/fast flick constants. WHY: Android can batch MOVE samples and the
  read-only capture renderer can wait on coarse polling, making the phone look
  choppy while tmux/laptop movement is already smooth. The APK now processes
...
- v2.64 restores the native prompt composer above the APK buttons and compacts
