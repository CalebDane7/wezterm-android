...
check_browser_overlay_gate() {
    ...
    if grep -Eq 'WEZTERM_WEB_OVERLAY|phone-web-overlay|parsed.path == "/web"|parsed.path == "/app.js"|parsed.path == "/app.css"' \
        ...
        require "$CONTROL_SERVER" 'showActive(){const payload=await api("/tabs")' "browser Active picker must avoid the Old Sessions sessions payload"
...
require "$PHONE_TERMINAL" 'PHONE_TITLE_SYNC_ENABLED:-0' "mutating title sync must remain opt-in during performance/browser work"
...
require "$CONTROL_SERVER" 'showActive(){const payload=await api("/tabs")' "browser Active picker must stay off the slower Old Sessions payload"
require "$CONTROL_SERVER" 'el.dataset.windowId' "browser overlay must bind Active rows to explicit tmux window ids"
...
require "$MAIN" 'toolbarNavigationButton("Refresh"' "APK toolbar must still expose Refresh"
require "$MAIN" 'toolbarNavigationButton("Bottom"' "APK toolbar must still expose Bottom"
require "$MAIN" 'toolbarNavigationButton("Scroll"' "APK toolbar must still expose Scroll"
