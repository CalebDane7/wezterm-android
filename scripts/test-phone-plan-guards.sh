...
require "$MANIFEST" 'android:versionName="1.54"' "current APK version must be bumped for the touch-scroll, Start/Stop, and active-switch fix"
...
if [ -f "$README" ]; then
    ...
    require "$README" 'Built checkpoint: `versionCode=55`, `versionName=1.54`.' "README checkpoint must match the installed v1.54 APK"
...
if [ "${PHONE_SKIP_GENERATED_PAGE_GUARD:-0}" != "1" ] && [ -f "$INSTALL_PAGE" ]; then
    require "$INSTALL_PAGE" 'WEzterm v1.54' "install page must advertise the current v1.54 APK"
    require "$INSTALL_PAGE" 'versionCode: <code>55</code>' "install page versionCode must match the manifest"
...
if [ "${PHONE_SKIP_GENERATED_PAGE_GUARD:-0}" != "1" ] && [ -f "$INSTALL_INDEX" ]; then
    require "$INSTALL_INDEX" 'WEzterm v1.54 Install' "install redirect page must not point users at a stale version label"
fi
...
if [ -f "$MENU_UI_PROOF" ]; then
    ...
    require "$MENU_UI_PROOF" 'for label in Active Old New Refresh Scroll "Copy/Paste" Upload Close Start Stop' "menu UI proof must check every bottom toolbar label"
...
require "$MAIN" 'restoreTouchLiveBottom' "finger-up bottom restore must use the lightweight touch path"
...
require "$MAIN" 'TYPE_TEXT_VARIATION_VISIBLE_PASSWORD' "terminal IME must avoid predictive/prose input behavior"
require "$MAIN" 'TYPE_TEXT_FLAG_NO_SUGGESTIONS' "terminal IME must disable suggestions where supported"
require "$MAIN" 'IME_ACTION_SEND' "terminal IME action must advertise send"
require "$MAIN" 'IME_FLAG_NO_PERSONALIZED_LEARNING' "terminal IME must request no personalized learning"
